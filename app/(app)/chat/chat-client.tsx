...
import { Bell, Camera, Check, Loader2, LogOut, Mic, Paperclip, Phone, Plus, Search, Send, Smartphone, Video, X } from "lucide-react";
...
export default function ChatClient({ currentUser }: { currentUser: User }) {
  ...
  const [pushStatus, setPushStatus] = useState("Уведомления выключены");
  ...
  async function enablePush() {
    ...
    const { publicKey, error } = await vapidResponse.json().catch(() => ({ publicKey: "" }));
    if (!vapidResponse.ok) {
      setPushStatus(error ?? "VAPID не настроен");
      return;
    }
      ...
      <aside className="glass flex max-h-[44dvh] flex-col rounded-[2rem] p-3 lg:max-h-none lg:w-[25rem]">
        <div className="flex items-center justify-between p-3">
          <div className="min-w-0">
            ...
            <p className="truncate text-xs text-slate-400">@{currentUser.username} · {currentUser.displayName}</p>
          </div>
          <button onClick={logout} className="rounded-2xl border border-white/10 bg-white/[0.06] p-3 text-slate-300 active:scale-95" aria-label="Выйти">
            <LogOut size={18} />
          </button>
        </div>

        {installTip ? (
          <div className="mx-3 mb-3 flex gap-3 rounded-3xl border border-cyan-300/20 bg-cyan-300/10 p-3 text-xs text-cyan-100">
            <Smartphone className="mt-0.5 shrink-0" size={16} />
            <span>{installTip}</span>
          </div>
        ) : null}

        <button onClick={enablePush} className="mx-3 mb-3 flex items-center justify-center gap-2 rounded-2xl border border-violet-300/20 bg-violet-400/10 px-4 py-3 text-sm font-semibold text-violet-100 active:scale-[0.98]">
          <Bell size={16} /> {pushStatus}
        </button>

        <form onSubmit={createChat} className="mx-3 mb-3 space-y-2">
          <div className="flex gap-2">
            <label className="flex min-h-12 flex-1 items-center gap-2 rounded-2xl border border-white/10 bg-white/[0.06] px-3 text-sm text-slate-300 focus-within:border-cyan-300/60">
              <Search size={16} />
              <input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Найти по @username" className="min-w-0 flex-1 bg-transparent outline-none placeholder:text-slate-500" autoCapitalize="none" />
            </label>
            <button className="rounded-2xl bg-white/[0.09] p-3 text-cyan-100 active:scale-95" aria-label="Открыть чат"><Plus size={19} /></button>
          </div>
          {searching ? <p className="px-2 text-xs text-slate-500">Ищу пользователя...</p> : null}
            ...
            <div className="space-y-2 rounded-3xl border border-white/10 bg-black/20 p-2">
              {searchResults.map((user) => (
                <button key={user.id} type="button" onClick={() => startPrivateChat(user.username)} className="flex w-full items-center gap-3 rounded-2xl p-2 text-left active:bg-white/[0.08]">
                  {user.avatarData ? <img src={user.avatarData} alt="" className="h-10 w-10 rounded-2xl object-cover" /> : <div className="grid h-10 w-10 place-items-center rounded-2xl bg-gradient-to-br from-fuchsia-500 via-violet-500 to-cyan-300 text-sm font-black">{avatarLabel(user.displayName || user.username)}</div>}
                  <span className="min-w-0 flex-1">
                    <span className="block truncate text-sm font-bold">{user.displayName}</span>
                    <span className="block truncate text-xs text-cyan-200">@{user.username}</span>
                  </span>
                  <span className="rounded-xl bg-cyan-300/10 px-2 py-1 text-xs font-semibold text-cyan-100">Чат</span>
                </button>
              ))}
            </div>
          ) : null}
        ...
        <div className="no-scrollbar flex-1 space-y-2 overflow-y-auto p-2">
          {loadingChats ? <p className="p-3 text-sm text-slate-400">Загружаю чаты...</p> : null}
          {chats.map((chat) => {
            ...
            return (
              <button key={chat.id} onClick={() => setActiveChatId(chat.id)} className={`w-full rounded-3xl p-3 text-left transition ${active ? "bg-white/[0.12]" : "bg-white/[0.04] active:bg-white/[0.08]"}`}>
                <div className="flex items-center gap-3">
                  {chat.avatarData ? <img src={chat.avatarData} alt="" className="h-12 w-12 shrink-0 rounded-2xl object-cover" /> : <div className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-gradient-to-br from-fuchsia-500 via-violet-500 to-cyan-300 font-black">{avatarLabel(chat.title)}</div>}
                  <div className="min-w-0 flex-1">
                    <p className="truncate font-semibold">{chat.title || "Чат"}</p>
                    <p className="truncate text-xs text-slate-400">{lastMessage?.text || (lastMessage?.type === "IMAGE" ? "Фото" : lastMessage?.type === "VIDEO" ? "Видео" : "Нет сообщений")}</p>
                  </div>
                </div>
              </button>
            );
      ...
      <section className="glass flex min-h-[56dvh] flex-1 flex-col overflow-hidden rounded-[2rem] lg:min-h-0">
        <header className="flex items-center justify-between border-b border-white/10 p-4">
          <div className="min-w-0">
            ...
            <p className="truncate text-xs text-slate-400">{activeChat?.username ? `@${activeChat.username}` : "mobile-first · PWA · login/password"}</p>
          </div>
          <div className="flex gap-2">
            <button onClick={() => startCall("AUDIO")} className="rounded-2xl border border-white/10 bg-white/[0.06] p-3 text-slate-300 active:scale-95" title="Аудиозвонок">
              <Phone size={18} />
            </button>
            <button onClick={() => startCall("VIDEO")} className="rounded-2xl border border-white/10 bg-white/[0.06] p-3 text-slate-300 active:scale-95" title="Видеозвонок">
              <Video size={18} />
            </button>
          </div>
        </header>
        ...
        {activeCall ? (
          <div className="mx-4 mt-4 flex items-center justify-between rounded-3xl border border-cyan-300/20 bg-cyan-300/10 p-3 text-sm text-cyan-100">
            <div>
              <p className="font-black">{activeCall.kind === "VIDEO" ? "Видеозвонок" : "Аудиозвонок"}</p>
              <p className="text-xs text-cyan-100/70">{activeCall.callerId === currentUser.id ? "Вы звоните" : `${activeCall.caller.displayName} звонит`} · {activeCall.status}</p>
            </div>
            <div className="flex gap-2">
        ...
        {callNotice ? <p className="mx-4 mt-3 rounded-2xl border border-white/10 bg-white/[0.06] p-3 text-xs text-slate-300">{callNotice}</p> : null}
          ...
          {messages.map((message) => {
            ...
            return (
              <div key={message.id} className={`flex ${mine ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-[86%] overflow-hidden rounded-[1.4rem] px-4 py-3 text-sm shadow-xl ${mine ? "bg-gradient-to-br from-violet-500 to-cyan-500 text-white" : "bg-white/[0.08] text-slate-100"}`}>
                  {!mine ? <p className="mb-1 text-xs font-semibold text-cyan-200">{message.sender?.displayName || message.sender?.username || "Pirogram"}</p> : null}
                  ...
                  <p className={`mt-1 text-right text-[10px] ${mine ? "text-white/70" : "text-slate-500"}`}>{timeLabel(message.createdAt)}</p>
                </div>
              </div>
            );
          })}
        ...
        {mediaDraft ? (
          <div className="border-t border-white/10 px-3 pt-3">
            <div className="flex items-center gap-3 rounded-3xl border border-white/10 bg-white/[0.06] p-3">
              ...
              <button onClick={() => setMediaDraft(null)} className="rounded-2xl bg-white/[0.08] p-2 text-slate-300"><X size={17} /></button>
            </div>
          </div>
        ) : null}
        ...
        <form onSubmit={sendMessage} className="flex items-end gap-2 border-t border-white/10 p-3">
          ...
          <button type="button" onClick={() => fileRef.current?.click()} className="rounded-2xl border border-white/10 bg-white/[0.06] p-3 text-slate-300 active:scale-95" title="Фото или видео">
            <Paperclip size={19} />
          </button>
          <button type="button" onClick={() => fileRef.current?.click()} className="rounded-2xl border border-white/10 bg-white/[0.06] p-3 text-slate-300 active:scale-95" title="Камера">
            <Camera size={19} />
          </button>
          <button type="button" className="rounded-2xl border border-white/10 bg-white/[0.06] p-3 text-slate-300 opacity-50" title="Голосовые сообщения можно добавить следующим файлом">
            <Mic size={19} />
          </button>
          <textarea value={text} onChange={(event) => setText(event.target.value)} placeholder="Сообщение" className="max-h-36 min-h-12 flex-1 resize-none rounded-2xl border border-white/10 bg-white/[0.06] px-4 py-3 text-sm outline-none placeholder:text-slate-500 focus:border-cyan-300/70" rows={1} />
          <button disabled={sending || (!text.trim() && !mediaDraft) || !activeChatId} className="rounded-2xl bg-gradient-to-br from-violet-500 to-cyan-400 p-3 text-white shadow-lg shadow-violet-950/40 disabled:opacity-50" aria-label="Отправить">
